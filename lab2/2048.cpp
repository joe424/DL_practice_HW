...
/*
 weight url: https://drive.google.com/file/d/1Li3jU84eoKuzhxvlORTn1YY2jiRALPVv/view?usp=sharing
*/
...
public:
	...
	virtual float estimate(const board& b) const {
		...
		for (int i = 0; i < iso_last; i++) {
			/* iso_last為該pattern加上其isomorphic的數量 */
			/* ex:{0, 1, 2, 3, 4, 5}的其中之一的isomorphic是{12, 8, 4, 0, 13, 9} */
			/* indexof返回該pattern(ex: {0, 1, 2, 3, 4, 5})對應的index */
			/* 假設說現在tile 0上有2，tile 1 上有4 */
			/* 因為2是2的'1'次方，4是2的'2'次方，1以2-bit表示是0001，2是0010 */
			/* 			 5    4    3    2    1    0    */
			/* 接起來 => 0000 0000 0000 0000 0010 0001 */
			/* 轉回十進位 => 100001 => index = 33 */
...
public:
	...
	/* ( s ---(r)---> s' ) ==> ( s'' ---(r_next)---> s'next ) ==> ( end )
    第一輪: 
            exact一開始為0，所以可以看成r_next跟V(s'')去比較，算出error，用error的值去調整V(s'')
            exact拿到V(s'')(調整後)並傳給下一輪
    第二輪:	
			再來error = V(s'') - (V(s) - r) = r + V(s'') - V(s)，用error的值去調整V(s)
			exact拿到V(s)(調整後)並傳給下一輪
    */
