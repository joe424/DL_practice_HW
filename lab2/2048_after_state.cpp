...
public:
    ...
    第一輪: 
            value的值是V(s'_next)+r_next，從value將r_next扣掉變成只剩V(s'_next)
            V(s'_next)跟0算出error，用error去調整V(s'_next)
            exact拿到V(s'_next)+r_next的值(V(s'_next)為調整後)並傳給下一輪
    第二輪:
            value的值是V(s')+r，從value將r扣掉變成只剩V(s')
            V(s')跟V(s'_next)+r_next算出error，用error去調整V(s')
            exact拿到V(s')+r的值(V(s')為調整後)並傳給下一輪
    */
